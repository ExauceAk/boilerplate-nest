...
export class UserCodeService {
    ...
    async getUserCodeByUserEmail(email: string): Promise<UsersCode> {
        ...
        const isUserCodeExist =
            await this.usersService.userCodeRepository.findOne({
                where: {
    ...
    async verifyUserCode(
        verifyUserCode: VerifyUserCodeDto,
    ): Promise<UsersCode> {
        ...
        const isCodeMatched =
            await this.usersService.hashService.comparePassword(
                code,
    ...
    async createUserCode(user: Users) {
        this.usersService.logger.info(
            `Creating user code with data: ${user.id}`,
        );
        const isUserCodeExist =
            await this.usersService.userCodeRepository.findOne({
    ...
    async askNewCode(user: Users) {
        ...
        if (
            isUserCodeExist.delayDate &&
            isUserCodeExist.delayDate > new Date()
        ) {
            ...
            const delayInHours = Math.ceil(
                (delayDate.getTime() - currentDate.getTime()) /
                    (1000 * 60 * 60),
            );
